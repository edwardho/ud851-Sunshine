    ...
    synchronized public static void initialize(@NonNull final Context context) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                // We have to check the ContentProvider to see if there is data
                // in our background AsyncTask

                // We only need to use the projection columns as we do not need
                // to actually display any data
                ...
                // If cursor is null or empty, there is no data populated and we need to sync
                if (cursor == null || cursor.getCount() == 0) {
                    startImmediateSync(context);
                }

                // Close cursor to avoid memory leaks when done
                cursor.close();
